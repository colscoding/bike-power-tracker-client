...
export class AnalyticsEngine {
    ...
    calculateWorkoutSummary(workout) {
        ...
        const powers = measurements.filter((m) => m.power != null).map((m) => m.power);

        const heartrates = measurements.filter((m) => m.heartrate != null).map((m) => m.heartrate);

        const cadences = measurements.filter((m) => m.cadence != null).map((m) => m.cadence);
    ...
    async findRecords() {
        ...
        for (const workout of workouts) {
            ...
            if (
                ...
                summary.normalizedPower > records.normalizedPower.value
            ) {
                records.normalizedPower = {
