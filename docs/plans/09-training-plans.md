...
export const workoutLibrary = [
  {
    ...
    segments: [
      ...
      },
      {
        name: 'Main Set',
        duration: 2700,
        target: { powerLow: 0.55, powerHigh: 0.70, cadenceLow: 85, cadenceHigh: 95 },
        instructions: 'Steady endurance pace, stay relaxed'
      },
      {
        name: 'Cool Down',
        duration: 300,
        target: { powerLow: 0.40, powerHigh: 0.50 },
    ...
    segments: [
      {
        name: 'Warm Up',
        duration: 900,
        target: { powerLow: 0.50, powerHigh: 0.65 },
      ...
      ...Array.from({ length: 10 }, (_, i) => [
        {
          name: `Hard ${i + 1}`,
          duration: 30,
          target: { powerLow: 1.10, powerHigh: 1.25 },
...
export class WorkoutProfile {
  ...
  getZoneColor(intensity) {
    ...
    if (intensity < 0.90) return '#FFEB3B'; // Z3 - Yellow
    if (intensity < 1.05) return '#FF9800'; // Z4 - Orange
    if (intensity < 1.20) return '#FF5722'; // Z5 - Deep orange
...
export class PlanTracker {
  ...
  load() {
    ...
    return allProgress[this.planId] || {
      startedAt: null,
      currentWeek: 1,
      completedWorkouts: []
    };
  ...
  getCompletionPercentage(plan) {
    ...
    return Math.round(
      (this.progress.completedWorkouts.length / totalWorkouts) * 100
    );
  }
}
```
